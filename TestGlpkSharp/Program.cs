...
namespace TestGlpkSharp
{
    class Program
    {
        ...
        private static void TestDumb()
        {
            var lp = new LPProblem();
            lp.Name = "REQUETE";

            lp.AddRows(17);

            lp.SetRowName(1, "COUT");
            lp.ObjectiveDirection = OptimisationDirection.MINIMISE;
            lp.SetRowStatus(1, BASESTATUS.Basic);

            lp.ObjectiveName = "COUT";
            lp.SetRowName(9, "CNT_PDIN2");
            lp.SetRowBounds(9, BOUNDSTYPE.Upper, 0, -0);
            lp.SetRowStatus(9, BASESTATUS.NonBasicUpper);
            lp.AddCols(3);

            lp.SetColName(1, "QUANTITE[MAIS]");
            lp.SetColBounds(1, BOUNDSTYPE.Lower, 0, 0);
            lp.SetColStatus(1, BASESTATUS.Basic);

            lp.SetColName(3, "QUANTITE[SOJA]");
            lp.SetColBounds(3, BOUNDSTYPE.Double, 0, 0);
            lp.SetColStatus(3, BASESTATUS.NonBasicFixed);

            lp.SolveSimplex();
            //'''''''test.PostSolve(lp, SOLUTIONTYPE.BasicSolution)''''
            lp.WriteSol("addrow.txt");
        }
    }
}
